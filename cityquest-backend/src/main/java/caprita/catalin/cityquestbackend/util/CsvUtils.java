...
public class CsvUtils {
    ...
    private void insertLocations(String[] locations){
        LOGGER.debug("BootStrapping Locations from a DTO.");
        int category = NATURE;
        int categoryIndex = 0;
    ...
    private void insertUsers(List<UserAnswerDto> dtos){
        dtos.forEach(dto -> {
            ...
            //Persist the user
            final User finalUser = userService.create(newUser);
            LOGGER.debug("SUCCESSFULLY persisted new User with timeStamp {}. Id {} ", newUser.getUsername(),
                    newUser.getId());
            //Add Ratings
            ...
            if(dto.getRestaurantsRecs() != null &&!dto.getRestaurantsRecs().isEmpty()) {
                Arrays.stream(dto.getRestaurantsRecs().split(",")).forEach(rec -> {
                    if(!rec.equals("0"))
                        addUserRecommendation(LocationCategory.RESTAURANTS,rec, finalUser.getId());
                });
            }
            if(dto.getClubsRecs() != null && !dto.getClubsRecs().isEmpty()) {
                Arrays.stream(dto.getClubsRecs().split(",")).forEach(rec -> {
                    if(!rec.equals("0"))
                        addUserRecommendation(LocationCategory.CLUBS,rec, finalUser.getId());
                });
            }
    ...
    private void addUserRecommendation(LocationCategory category,String recName, Long userId){
        Location location = locationService.findByName(recName);
        if(location == null){
        ...
        LOGGER.debug("Location {}  " ,location.getName());
        ratingService.addUserLocationRating(userId,
                location.getId(),
                3);
    }
